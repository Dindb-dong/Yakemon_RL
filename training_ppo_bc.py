...
# 모델 관련 import
from ppo_bc import MaskablePPOBC
...
from ppo_bc import MaskableBCActorCriticPolicy

# PPO용 하이퍼파라미터 설정
HYPERPARAMS_PPO = {
    "policy": MaskableBCActorCriticPolicy,
    "learning_rate": 3e-4,
    "n_steps": 1024, # 각 업데이트 전에 실행할 스텝 수
    "batch_size": 128, # PPO 미니배치 크기
    ...
    "vf_coef": 0.5,
    "max_grad_norm": 0.5,
    "total_timesteps": 5000000, # 총 학습 타임스텝
    "save_freq": 500000, # 타임스텝 기준 저장 빈도
    "test_episodes": 300,
...
if __name__ == "__main__":
    ...
    results_dir = os.path.join('results', f'ppo_bc_traing_{timestamp}')
    models_dir = os.path.join('models', f'ppo_bc_training_{timestamp}')
    
    # 환경 초기화
    env = YakemonEnv()

    # PPO 에이전트 생성
    ppo_agent = MaskablePPOBC(
