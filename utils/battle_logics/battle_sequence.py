...
def battle_sequence(
    ...
    if my_action is None and enemy_action is not None:
        ...
        if is_move_action(enemy_action):
            result: dict[str, Union[bool, int]] = handle_move("enemy", enemy_action, active_enemy, watch_mode)
        elif is_switch_action(enemy_action):
            switch_pokemon("enemy", enemy_action["index"])
    ...
    if enemy_action is None and my_action is not None:
        ...
        if is_move_action(my_action):
            handle_move("my", my_action, active_my, watch_mode)
        elif is_switch_action(my_action):
            switch_pokemon("my", my_action["index"])
        apply_end_turn_effects()
    ...
    if is_switch_action(my_action):
        ...
        if is_move_action(enemy_action):
            if enemy_action.name == "기습":
                ...
                print('나는 교체, 상대는 공격!')
                result: dict[str, Union[bool, int]] = handle_move("enemy", enemy_action, store.get_active_index("enemy"), watch_mode, True)
        apply_end_turn_effects()
        return result if result else {"was_null": False, "was_effective": 0}
    ...
    if is_switch_action(enemy_action):
        ...
        if is_move_action(my_action):
            if my_action.name == "기습":
                ...
                print('상대는 교체, 나는 공격!')
                handle_move("my", my_action, store.get_active_index("my"), watch_mode, True)
        apply_end_turn_effects()
        return {"was_null": False, "was_effective": 0}
    ...
    if is_move_action(my_action) and is_move_action(enemy_action):
        if who_is_first == "my":
            if my_action.name == "기습" and enemy_action.category == "변화":
                ...
                store.add_log("enemy의 기습은 실패했다...")
                print("enemy의 기습은 실패했다...")
                handle_move("my", my_action, store.get_active_index("my"), watch_mode)
            else:  # 그 외의 일반적인 경우들
                print('내 선공!')
                handle_move("my", my_action, store.get_active_index("my"), watch_mode)
                # 상대가 쓰러졌는지 확인
                ...
                if current_defender and current_defender.current_hp <= 0:
                    apply_end_turn_effects()
                    return result if result else {"was_null": False, "was_effective": 0}
                result: dict[str, Union[bool, int]] = handle_move("enemy", enemy_action, active_enemy, watch_mode, True)
        else:  # 상대가 선공일 경우
            if enemy_action.name == "기습" and my_action.category == "변화":
                ...
                store.add_log("my의 기습은 실패했다...")
                print("my의 기습은 실패했다...")
                result: dict[str, Union[bool, int]] = handle_move("enemy", enemy_action, store.get_active_index("enemy"), watch_mode)
            else: # 일반적인 경우 
                print('상대의 선공!')
                result: dict[str, Union[bool, int]] = handle_move("enemy", enemy_action, store.get_active_index("enemy"), watch_mode)
                ...
                if current_defender and current_defender.current_hp <= 0:
                    apply_end_turn_effects()
                    return result
                handle_move("my", my_action, active_my, watch_mode, True)
...
def handle_move(
    ...
    if is_triple_hit:  # 트리플악셀, 트리플킥
        ...
        for i in range(hit_count):
            ...
            result: dict[Literal["success", "damage", "was_null", "was_effective"], Union[bool, int]] = calculate_move_damage(
                move_name=move.name,
                side=side,
            ...
            if result and result["success"]:
                ...
                apply_after_damage(side, attacker, current_defender1, move, result["damage"] if "damage" in result else 0, watch_mode, True)
                apply_defensive_ability_effect_after_multi_damage(side, attacker, defender, move, result["damage"] if "damage" in result else 0, watch_mode)
            else:
                break
        ...
        result: dict[Literal["success", "damage", "was_null", "was_effective"], Union[bool, int]] = calculate_move_damage(move_name=move.name, side=side, current_index=current_index, was_late=was_late)
        print("1번째 타격!")
        if result and result["success"]:
            ...
            apply_after_damage(side, attacker, current_defender, move, result["damage"] if "damage" in result else 0, watch_mode, True)
            hit_count = get_hit_count(move)
            print(hit_count)
            ...
            apply_move_effect_after_multi_damage(side, attacker, current_defender1, move, result["damage"] if "damage" in result else 0) #, watch_mode
        ...
        if result and result["success"]:
            ...
            if defender and defender.base.ability and defender.base.ability.name == "매직가드" and move.category == "변화":
                ...
                apply_after_damage(side, attacker, defender, move, result["damage"] if "damage" in result else 0, watch_mode)
                return {"was_null": result.get("was_null", False), "was_effective": result.get("was_effective", 0)}

            current_defender = store.get_state()[f"{opponent_side}_team"][
                active_enemy if side == "my" else active_my
            ]   
            apply_after_damage(side, attacker, current_defender, move, result["damage"] if "damage" in result else 0, watch_mode)

        return {"was_null": result.get("was_null", False), "was_effective": result.get("was_effective", 0)}

def remove_fainted_pokemon(side: Literal["my", "enemy"]) -> None:
    next_index = get_best_switch_index(side)
    if next_index != -1:
        print(f"{side}의 포켓몬이 쓰러졌다! 교체 중...")
        switch_pokemon(side, next_index)
